...
import { AwardsTypes } from "@/components/sections/AwardsTypes";
...
export const AwardSection: React.FC<AwardSectionProps> = ({
  ...
  const visibleAwards = Award.slice(0, visibleCount);
  const hasMore = visibleCount < Award.length;
    ...
    <div className="flex flex-col items-start justify-start w-full max-w-[1200px]">
      <div className="flex flex-col w-full divide-y divide-white/20">
        {visibleAwards.map((item, index) => (
          <div
            key={index}
            className="flex items-start justify-start w-full gap-8 py-14"
          >
            <div className="flex flex-col items-start gap-2 min-w-[300px]">
              <span className="text-white text-2xl uppercase">{item.year}</span>
              <span className="text-white/60 text-3xl md:text-4xl GeistBold">
                {item.position}
              </span>
            </div>
            <div className="flex flex-col items-start gap-2 min-w-[300px]">
              ...
              <span className="text-white/60 text-3xl md:text-4xl GeistBold leading-12">
                {item.para}
              </span>
            </div>
          </div>
